...
We first collect the data on the SSB's website and store it as a JSON. This will avoid the problems that one would have with this data set if they were to use the CSV format instead.
```{r}
...
Then, the content is retrieved as text, and processed by the 'fromJSONstat' function which takes a JSON file and returns a list of data frames, so we use the double square brackets to retrieve only the data and not the entire data-frame.
```{r}
...
```{r include=TRUE} 
ggplot(to_plot, aes(x=month(date), y=difference)) + geom_bar(stat = "identity") +
  xlab("Month") + 
